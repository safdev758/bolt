...
router.post('/save-babysitter', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    // Prevent duplicates
    if (mother.saved_babysitters.includes(babysitterId)) {
      return res.status(409).json({ message: 'Babysitter already in favorites' });
    }

    mother.saved_babysitters.push(babysitterId);
    await mother.save();

    res.status(200).json({ message: 'Babysitter added to saved', saved: mother.saved_babysitters });
  } catch (error) {
    console.error(error);
