...
router.post('/add-child-offline', auth, async (req, res) => {
    ...
    if (!name || !age || !babysittingDate?.month || !babysittingDate?.day || !babysittingDate?.time) {
      return res.status(400).json({ message: 'Incomplete child or date info' });
    }
  
    try {
      const newRequest = new BabysittingRequest({
        mother: '661dbfc37982954f1cf429ab',  // hardcoded guest mother ID
      ...
      res.status(201).json({ message: 'Child added (offline) successfully', request: newRequest });
    } catch (err) {
      console.error(err);
